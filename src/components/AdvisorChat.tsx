...
function ruleReply(q: string, lang: Lang, gpaStr: string, credits: number, plan: { doneCredits: number; reqCredits: number; remainingCredits: number; pct: number; gradTerm: string }) {
  ...
  const R: Record<string, { en: string; ar: string }> = {
    ...
    video: {
      en: "For Operating Systems start with the freeCodeCamp OS course, and for Algorithms use the freeCodeCamp Data Structures course — both are pinned under Resources → Video sources.",
      ar: "لنظم التشغيل ابدئي بدورة freeCodeCamp، وللخوارزميات استخدمي دورة هياكل البيانات — كلاهما مثبّت في المصادر ← مصادر الفيديو.",
    },
    def: {
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 18, height: "100%", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,#1E8378,#2C6E91)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}><Icon name="auto_awesome" size={22} /></div>
        ...
        {onClose && (
          <button onClick={onClose} aria-label="Close" style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 9, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--muted)", flexShrink: 0 }}><Icon name="close" size={19} /></button>
        )}
      </div>

      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12, background: "var(--surface-2)" }}>
        {display.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={m.role === "user"
              ? { maxWidth: "82%", background: "#102A40", color: "#fff", borderTopLeftRadius: 14, borderTopRightRadius: 14, borderBottomLeftRadius: 14, borderBottomRightRadius: 4, padding: "11px 14px", fontSize: 13.5, lineHeight: 1.5, whiteSpace: "pre-wrap" }
              : { maxWidth: "85%", background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", borderTopLeftRadius: 14, borderTopRightRadius: 14, borderBottomLeftRadius: 4, borderBottomRightRadius: 14, padding: "11px 14px", fontSize: 13.5, lineHeight: 1.55, whiteSpace: "pre-wrap" }}>
              {m.content}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: "flex", gap: 9, padding: "12px 14px", borderTop: "1px solid var(--border)", background: "var(--surface)" }}>
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={t.askPlaceholder} style={{ flex: 1, border: "1px solid var(--border)", borderRadius: 11, padding: "11px 13px", fontSize: 14, outline: "none", color: "var(--text)", background: "var(--surface-2)" }} />
        <button onClick={() => send()} aria-label="Send" style={{ width: 44, height: 44, borderRadius: 11, background: "#102A40", color: "#fff", border: "none", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon name={lang === "ar" ? "arrow_back" : "send"} size={20} /></button>
      </div>
    </div>
  );
}
