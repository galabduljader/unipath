...
export default function ResourcesPage() {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", flexDirection: "column", gap: 22 }} className="fade-up">
      ...
      <div>
        <div className="serif" style={{ fontSize: 21, fontWeight: 600, color: "#102A40" }}>{t.videoSources}</div>
        <div style={{ fontSize: 13, color: "#6E7C86", marginBottom: 14 }}>{t.videoSourcesSub}</div>
        <div style={{ display: "grid", gridTemplateColumns: grid, gap: 14 }}>
          {vids.map((v, i) => (
            <div key={i} style={{ border: "1px solid #E7E0D3", borderRadius: 14, overflow: "hidden", background: "#fff" }}>
              ...
              <div style={{ padding: "13px 15px" }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#102A40", lineHeight: 1.3 }}>{v.title}</div>
                <div style={{ fontSize: 12, color: "#9aa6ad", marginTop: 4, display: "flex", alignItems: "center", gap: 5 }}><Icon name="smart_display" size={14} />{v.source}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <div className="serif" style={{ fontSize: 21, fontWeight: 600, color: "#102A40" }}>{t.tutors}</div>
        <div style={{ fontSize: 13, color: "#6E7C86", marginBottom: 14 }}>{t.tutorsSub}</div>
        <div style={{ display: "grid", gridTemplateColumns: grid, gap: 14 }}>
          {tts.map((tu, i) => (
            <div key={i} style={{ border: "1px solid #E7E0D3", borderRadius: 14, padding: 16, background: "#fff", display: "flex", alignItems: "center", gap: 13 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#102A40" }}>{tu.name}</div>
                <div style={{ fontSize: 12, color: "#6E7C86", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tu.focus}</div>
                <div style={{ fontSize: 11.5, color: "#9aa6ad", marginTop: 3, display: "flex", alignItems: "center", gap: 8 }}>
