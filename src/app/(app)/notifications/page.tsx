  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 14 }} className="fade-up">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div className="serif" style={{ fontSize: 23, fontWeight: 600, color: "#102A40" }}>{t.notifsTitle}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {notifications.map((n) => {
          const ic = ICONS[n.icon] || ["#F4EEE3", "#6E7C86"];
          return (
            <button key={n.id} onClick={() => { openNotification(n); if (n.link) router.push("/" + n.link); }} style={{ display: "flex", gap: 13, alignItems: "flex-start", padding: "15px 16px", border: `1px solid ${!n.read ? "#CDE0EC" : "#E7E0D3"}`, borderRadius: 13, background: !n.read ? "#F5FAFC" : "#fff", width: "100%" }}>
              ...
              <div style={{ flex: 1, minWidth: 0, textAlign: "start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontWeight: 600, fontSize: 14, color: "#102A40" }}>{n.title}</span>
                  {!n.read && <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#1E8378" }} />}
                </div>
                {n.body && <div style={{ fontSize: 13, color: "#6E7C86", marginTop: 2 }}>{n.body}</div>}
                <div style={{ fontSize: 11.5, color: "#9aa6ad", marginTop: 5 }}>{relTime(n.created_at, lang)}</div>
              </div>
            </button>
          );
        })}
        {notifications.length === 0 && (
          <div style={{ textAlign: "center", padding: 40, color: "#9aa6ad", fontSize: 13 }}>—</div>
        )}
      </div>
    </div>
  );
